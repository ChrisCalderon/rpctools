...
rpctools.httprpc.HEADERS['User-Agent'] = rpctools.httprpc.HEADERS['User-Agent'].format(__version__)


def rpc_factory(address, verbose):
    """Chooses the appropriate rpc class based on the address format.

    Does not work for EtherscanRPC because there is no good way to check if an api_key is valid.
    If you know you have an apikey for Etherscan, instantiate the EtherscanRPC client directly."""
